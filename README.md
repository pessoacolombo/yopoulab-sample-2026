...
| **3** | **`3a_Morphometrics`** | **Computes urban morphometrics** at building-, tessellation- and street- levels | `data/temp/*_momepy_bdg_part{1,2}of2.parquet`, `*_momepy_tess_part{1..4}of4.parquet`, `streets.gpkg`, `nodes.gpkg`, `edges.gpkg` |
| *3.1* | **`3b_Spatial-deprivation-score-representativity`** | **Part A:** scores every building with a graded **spatial deprivation score (−1..8)**. **Part B: audits the diversity and representativity of sampled areas** (in terms of morphometrics) against the AOI (HDSS frame) and against Yopougon as a whole (expansion frame) | `data/output/*buildings_spatial-deprivation-scored.parquet`, `*blocks_spatial-deprivation-scored.gpkg` |

*Some `data/temp/` and `data/output/` files are GeoParquet (`.parquet`) instead of `.gpkg`, and the two
largest are split into several parts — see `data/README.md` for exactly what changed and why (all for
GitHub's 25 MB file-size limit; no data was altered beyond format/compression, except the two morphometrics
files and the raw Cocody-Bingerville footprints, which are restricted to / dropped outside the
Yopougon-Attécoubé scope this repo actually uses).*
...
`matplotlib`, `pyarrow` (reads/writes the GeoParquet files in `data/` — see the note under *Workflow* above).
All spatial work uses **EPSG:2041** (Abidjan 1987 / UTM 30N) so distances and areas are metric.
